...
DATASETS = {
    ...
    # PATCH (edgefm-eqc): LongBench-v2 long-context QA. The cache jsonl is normally
    # pre-built with a tokenizer-exact 8192-token context truncation (see
    # scripts/prepare_longbench_bench.py); this char fallback (~4 char/tok) only
    # applies if the cache is missing. Needs --max-model-len >= ~9216 to serve.
    "longbench": {
        "load_args": ("zai-org/LongBench-v2",),
        "load_kwargs": {"split": "train"},
        "format": lambda x: x["context"][:32768] + "\n\n" + x["question"],
    },
}
...
def _check_transformers_model(model_name: str) -> None:
    # PATCH (edgefm-eqc): allow Qwen3.5 / local CT-bf16 target paths. The upstream
    # regex rejects "qwen3.5" and any path not literally containing qwen3/llama
    # (our target dir is local_model_ct_ckpt5000-bf16, model_type=qwen3_5). transformers
    # 5.x (.venv) loads qwen3_5 fine and dflash.model imports cleanly, so skip the gate.
    return
...
def _vllm_spec_metrics(base_url: str) -> tuple[float, float] | None:
    """PATCH (edgefm-eqc): read cumulative spec-decode counters from vLLM /metrics
    (Prometheus text) so the vllm backend can report acceptance length (MAL) — the
    OpenAI usage payload only has completion_tokens, not accept stats. Returns
    (num_drafts, num_accepted_tokens), or None if metrics are unavailable."""
    try:
        txt = requests.get(base_url + "/metrics", timeout=30).text
    except Exception:
        return None
    ...
    # Prometheus counters carry a _total suffix (… _created is a timestamp; exclude it)
    drafts = _sum("vllm:spec_decode_num_drafts_total")
...
def _run_server(args: argparse.Namespace) -> None:
    ...
    spec_before = _vllm_spec_metrics(args.base_url) if is_vllm else None
    ...
    spec_after = _vllm_spec_metrics(args.base_url) if is_vllm else None
    toks_per_s = total_tokens / max(latency, 1e-6)

    # PATCH (edgefm-eqc): derive MAL from /metrics delta for the vllm backend.
    vllm_mal = None
    if spec_before is not None and spec_after is not None:
        d_drafts = spec_after[0] - spec_before[0]
        d_accept = spec_after[1] - spec_before[1]
        if d_drafts > 0:
            vllm_mal = (d_accept + d_drafts) / d_drafts
    ...
    if vllm_mal is not None:
        print(f"Accept length:    {vllm_mal:.3f}   (MAL, from vLLM /metrics)")
