...
def destroy_distributed():
    ...
    # Some groups can be None or already unregistered depending on the parallel
    # layout (e.g. hf target backend without ulysses/ring SP). Passing None
    # would destroy the DEFAULT group first and every later destroy then
    # raises "Invalid process group specified" — guard each one and tear the
    # default group down last.
    for pg in (_TP_GROUP, _DP_GROUP, _SP_ULYSSES_GROUP, _SP_RING_GROUP,
               _DRAFT_DP_GROUP, _DRAFT_SP_GROUP):
        if pg is None:
            continue
        try:
