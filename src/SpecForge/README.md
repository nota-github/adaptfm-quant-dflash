> **Vendored into edgefm-eqc as `src/SpecForge/`** — trimmed to the DFlash training path
> this repo uses (`scripts/train_dflash.py` + the `specforge` package, `hf` target
> backend; local changes are recorded in `dflash_hf.patch`). Other upstream files
> referenced below (assets, `examples/`, `train_eagle3.py`, `train_domino.py`, docs)
> are **not in this tree**. The upstream README is kept for reference.
